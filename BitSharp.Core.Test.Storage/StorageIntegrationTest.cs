        ...
        private void TestRollback(ITestStorageProvider provider)
        {
            ...
            using (var chainStateBuilder = new ChainStateBuilder(rules, coreStorage, storageManager))
            {
                ...
                for (var blockIndex = 0; blockIndex < blocks.Count; blockIndex++)
                {
                    ...
                    chainStateBuilder.AddBlock(chainedHeader, block.Transactions.Select(
                        (tx, txIndex) => new BlockTx(txIndex, depth: 0, hash: tx.Hash, pruned: false, transaction: tx)));
                ...
                var expectedLastUtxoHash = UInt256.ParseHex("8e7a5912c6e9e46370841e54c590dab04f37861345ee9946b69e0c8505f602eb");
